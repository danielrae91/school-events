...
Return ONLY valid JSON in this format:
{
  ...
  const handleDedupeEvents = async () => {
    if (!confirm('Are you sure you want to remove duplicate events? This will find and delete similar events with 80%+ title similarity on the same date.')) return
    ...
    try {
      ...
      if (response.ok) {
        ...
        alert(`Deduplication completed: ${result.eventsDeleted} duplicate events removed`)
      }
    } catch (err) {
      console.error('Failed to dedupe events:', err)
      alert('Failed to remove duplicate events')
    }
  }
